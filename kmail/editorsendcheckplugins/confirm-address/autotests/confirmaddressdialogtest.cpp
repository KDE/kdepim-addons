...
void ConfirmAddressDialogTest::shouldHaveDefaultValue()
{
    ...
    const auto confirmAddressWidget = dlg.findChild<ConfirmAddressWidget *>(u"confirmwidget"_s);
    QVERIFY(confirmAddressWidget);

    const auto whiteListEmailsButton = dlg.findChild<QPushButton *>(u"whiteListEmailsButton"_s);
