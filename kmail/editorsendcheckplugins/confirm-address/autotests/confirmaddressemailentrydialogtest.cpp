...
void ConfirmAddressEmailEntryDialogTest::shouldHaveDefaultValue()
{
    ...
    const auto label = dlg.findChild<QLabel *>(u"label"_s);
    QVERIFY(label);
    QVERIFY(!label->text().isEmpty());

    const auto mLineEdit = dlg.findChild<QLineEdit *>(u"lineedit"_s);
    QVERIFY(mLineEdit);
    QVERIFY(mLineEdit->text().isEmpty());
    QVERIFY(mLineEdit->validator());
    QVERIFY(qobject_cast<const KEmailValidator *>(mLineEdit->validator()));

    const auto buttons = dlg.findChild<QDialogButtonBox *>(u"buttons"_s);
    QVERIFY(buttons);
}
