...
set in the current scope (since 6.21).
...
::
  ...
  ecm_test_set_dir_properties( # Since 6.21.0
      [DIR <dir>]
      [PREFIX_NAME <name>]
...
of dirs in the path used for deriving the prefix name.

::

  ecm_test_get_name_prefix( # Since 6.21.0
      <name_prefix_var>
  )
...
the value of ``ECM_TEST_NAME_PREFIX_STRATEGY`` and the respective further variables.

::

  ECM_TEST_NAME_PREFIX_STRATEGY # Since 6.21.0
...
when no explicit ``NAME_PREFIX`` argument is passed. The supported values are:
    ...
    * ``VARIABLE``: The default name prefix is taken from the value of the variable
      ``ECM_TEST_NAME_PREFIX`` in the scope where the macros is called.

    * ``PATH``: The default name prefix is derived from the relative path of the
      current source directory where the macro is called, by replacing the
      dir separators with "-".
...
::
  ...
  ECM_TEST_NAME_PREFIX_IGNORE_DIRS # Since 6.21.0
...
if(NOT ECM_TEST_NAME_PREFIX_IGNORE_DIRS)
...
function(ecm_test_get_name_prefix _name_prefix_varname)
    ...
    if (${ECM_TEST_NAME_PREFIX_STRATEGY} STREQUAL "PATH")
